...
// unless it is a known holdout (GNOME/Mutter). X11 and a missing or
// empty session type are reported as unsupported.
...
checkLayerShellCapability(const char *sessionType, const char *currentDesktop) {
    ...
    const bool isX11 = sessionType && std::strcmp(sessionType, "x11") == 0;
    ...
    // protocol (e.g. Weston) would be reported as supported here, but the
    // overlay daemon fails gracefully at startup in that case.
    cap.supported = true;
    return cap;
}
